...
- **미검증 후보**: ① 모듈 SDA/SCL↔GND 단락 또는 구멍 오인 ② `ESF-BCC`가 5 V 타입(데이터시트 전기 규격 4.5~5.5 V)인데
  3 V로 구동. ②를 보려고 5 V로 바꿨는데 **전원이 열화상 모듈에서 점프되는 배선이라 열화상에도 5 V가 걸렸다**
  (정격 최대 3.6 V). 열화상이 즉시 무응답 → 3.3 V 복귀 후 `0x33`·ID 정상 회복. 교훈: **전원은 모듈 간 점프 금지.**
