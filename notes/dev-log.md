...
- viewer에 스칼라 스트림 카드 추가: 세션이 `data_kind=scalar` 스트림 최신값을 무효(fault) 포함 캐시하고 기존
  `preview_array` 엔드포인트가 `kind:"scalar"`로 내보낸다(새 경로 없음). 큰 숫자 + 최근 120개 추이. 테스트 1개 추가, 59 통과.
- 수집 서비스(8000)를 PT100 설정(`CE0`/430/`wires=4`)으로 재시작. 22시간 열화상 세션은 정상 중지 후 `/tmp` → `~/collector-data` 이동.
- 통합 세션 10.5분: 열화상 1,256/1,256(1.994 Hz), PT100 631/631(1.001 Hz), 무효·드롭 0. 최대 공백 2.7 s / 3.2 s.
  30분·분리 재연결 시험은 사용자 판단으로 생략. 상세: `notes/data/experiments/20260926_thermal-pt100-integrated-run.md`.

## 2026-09-26 — MAX31865 해결: CS를 하드웨어 CS0(24번)로, PT100 첫 판독 29.2 ℃

- 배선(VIN 17 / GND 25 / CLK 23 / SDO 21 / SDI 19 / CS 16=`D23`), 원시 레지스터 시험(config `0xD3` 쓰기→되읽기,
  SPI 모드 4개)과 `sensor_check.py pt100` 모두 실패. 점퍼선 교체 전 전부 `0x00`, 교체 후 전부 `0xFF`.
- 19↔21 맞바꿈(SDO/SDI 표기 반대 가설) → 동일 `0xFF` — 가설 기각, 원복. 21번이 보드 입력(SDI)에 물려도 `0xFF`이므로
  `0xFF`는 MISO가 떠 있는 값이고 **칩은 SDO를 한 번도 구동하지 않았다.**
- **같은 모델(VLT-THM024) 새 모듈로 교체해도 동일** → 개별 모듈 불량 가능성 낮음. 헤더 납땜은 되어 있음(사용자 확인).
- **SCK 검증**: 23번↔15번(`D22` 입력) 점퍼 직결, 20 kHz 전송 중 입력 변화 49,804회/3 s 감지 → SCK 출력 정상.
  이로써 Jetson 쪽 MOSI·MISO(9/25 루프백)·SCK·CS(GPIO 출력) 전부 확인. 참고: 전송 없을 때 SCK 유휴 레벨이 HIGH였다.
- **어댑터 결함 발견**: `rtd_max31865.py`의 `_responds()`는 bias를 켠 뒤 1로 읽히는지만 봐서 MISO가 HIGH에 붙으면
  `connected=True` 가짜 양성이 난다 → 아래에서 수정.
- 보드 3V3 핀이 VIN 3.3 V 인가 중 **0 V**로 측정됨. 3V3에 직접 3.3 V를 넣자 `0x00`으로 바뀜. 최종 성공 구성은
  **VIN 3.3 V(3V3 비움)** 이므로 이 0 V는 무응답 원인이 아니었다(측정 조건 불명).
- **결정적 변화: CS를 24번(하드웨어 CS0)으로** → config 되읽기 `0xD1` 성공(SPI 모드 1/3). CS를 16번으로 되돌리면 다시
  `0x00` — 원인은 CS 핀. GPIO CS가 안 되는 이유는 미규명. → **D-034**: `CE0`이면 spidev 전이중 전송 드라이버
  (`_SpidevMax31865`)로 읽도록 어댑터 수정, `_responds()` 가짜 양성도 수정(bias 켜서 1·꺼서 0 확인). 테스트 4개 추가,
  전체 58 통과.
- PT100: 3선 결선(F− 비움)은 원시값 `32767`(회로 열림) — 보드 2/3선 납땜 점퍼가 출하 상태(4선). **RTD−↔F− 점퍼**로
  4선 모드 측정 → **10/10, 29.2 ℃, 111.36 Ω, 원시값 8486 ±1** → 기준 저항 430 Ω 확인.
  상세: `notes/data/experiments/20260926_max31865-pt100-first-read.md`.
- 뜨거운 물 담금 7분 연속 기록(1 Hz): **430/430 유효, fault 0**, 41.6 → 37.3 ℃ 냉각 곡선 정상 추종.
  기준 온도계가 없어 정확도는 미확인(실험 노트 추가 절).
- 남은 일: 2/3선 점퍼 납땜 후 `wires=3`, 2점(얼음물·끓는 물) 확인, GPIO CS 무응답 원인.
...
- **루프백으로 장치 번호 확정**: J12 19↔21번을 점퍼로 직접 잇고 보낸 값이 그대로 돌아오는지 본 결과
  **`/dev/spidev0.0`·`0.1`이 헤더**였다(`spidev1.x`는 무관, 전부 0x00). 헤더 이름 `spi1_*`과 장치 번호가 다르다 —
  `i2c8 → /dev/i2c-7`과 같은 함정. 배선안에 기록.
- **MAX31865는 응답하지 않는다**(미해결). 시험한 조합: 버스 2개 × CS 핀 2개(15=D22, 16=D23) × SPI 모드 4개.
  config 레지스터에 `0xD3`을 써도 읽으면 `0x00`(또는 `0xFF`)으로, 쓰기가 유지되지 않는다.
  Adafruit 드라이버 경로에서는 모든 레지스터가 `0xFF`로 읽혀 fault 비트가 전부 켜진 것처럼 보였다(= 통신 없음).
- 과정에서 확인한 것: GPIO 16번 출력 정상, 보드 VIN에 3.3 V 인가 확인(처음에는 전원이 안 들어와 있었고, CS도 처음엔
  미연결 상태였다 — 둘 다 해결한 뒤에도 무응답). 보드에 전원 LED는 보이지 않는다.
- **결론: Jetson 쪽(핀 설정·SPI·GPIO)은 정상.** 남은 원인은 **점퍼선 또는 MAX31865 보드**다. 다음 작업은
  4가닥 도통 확인·선 교체, 그래도 안 되면 보드 교체·납땜 헤더. 이번 주 세 센서(열화상·적외선·PT100) 모두
  같은 패턴(듀폰 접촉)으로 시간을 썼다.

## 2026-09-25 — 열화상 45.7시간 연속 운전 결과 + 깨진 프레임 범위 검사(D-032)

- 9/23에 시작한 열화상 세션이 **45.65시간**(2,739분) 돌았다. 지침서 §5-7(30분 연속)을 크게 넘겼다.
  **표본 328,354 · 무효 17(전부 `i2c_error: Remote I/O error`) · 성공률 99.995 % · 실효 1.998 Hz ·
  최대 공백 2.84초 · 취득 중앙 156 ms(최대 223) · 드롭 0.** 서비스는 멈추지 않았고 재시도 344건은 자체 복구됐다.
  상세: `notes/data/experiments/20260925_thermal-45h-continuous-and-range-check.md`.
- **문제 발견**: 9/23에 본 810 ℃ 1화소 스파이크가 우연이 아니었다. 32.8만 프레임에서 **282 프레임(0.086 %)에
  300 ℃ 초과 화소**(최대 862 ℃), 화소 단위 153 ppm. 1화소만 튄 것부터 **768화소 전부가 깨진 프레임**까지 있고
  약 800프레임(6~7분)에 한 번꼴이다. 음수 쪽 이상은 0건. **전부 `valid:true`로 저장되고 있었다.**
- **D-032 이행**: `thermal_mlx90640.py`에 `OBJECT_RANGE_C=(-40, 300)` 검사 추가 — 범위 밖 화소가 있으면 **재시도**하고,
  재시도를 다 쓰면 `valid:false` + `invalid_reason=out_of_range_pixels:N` + `flags.out_of_range`(수·최소/최대·위치 16개).
  **값은 손대지 않는다.** 깨진 프레임은 장치 고장으로 세지 않아 재연결로 번지지 않는다.
  `COLLECTOR_THERMAL_RANGE_C`로 조절(기본 `-40,300`), `applied_config.valid_range_c`로 세션 메타에 남는다.
- **검증**: 45.7시간 원본(1.0 GB)에 검사를 그대로 돌려 **282/328,958**을 판정 — 사전 계산과 정확히 일치.
  최악 프레임은 768화소가 425~432 ℃, 1화소 예는 (23,0)이 858 ℃. 테스트 2건 추가, 전체 **54건 통과**.
  실기기 재기동 후 30초 세션에서 `valid_range_c: [-40, 300]`이 메타에 남고 무효 0.
- **한계 기록**: 범위 안으로 그럴듯하게 깨진 프레임은 못 잡는다. 이웃/시간 대비 검사는 정상 급변을 버릴 위험이 있어 보류.
- 정리: 임시 폴더의 29 GB 점검 세션(9/23 `/viewer` 버튼으로 생긴 Gemini 2 depth/IR)을 사용자 승인 후 삭제. 디스크 383 GB 여유.
