...
> **구성 변경(2026-09-23, D-030·D-031):** MLX90614 2대는 **계획에서 제외**됐고(솥 내장 온도센서로 대체),
> 열화상도 **D55 1대만** 쓴다(D-031 — 같은 물체 비교에서 D55 42 % vs D110 10 %). 따라서 남은 대상은
> **MLX90640 1대 + MAX31865/PT100 1대**이고, **TCA9548A·100 kHz 버스(27/28번)는 쓰지 않는다**.
> §2·§5의 MLX90614 단계와 §5-3(열화상 2대·mux)은 보류다.
>
> **구현 상태(2026-09-23):** §4의 어댑터 3종·버스 잠금·`tools/sensor_check.py`를 작성했고 가짜 드라이버
> 테스트는 통과했다. §5-2(열화상 첫 대)는 **완료** — MLX90640 1대를 J12 1/3/5/6번(= `/dev/i2c-7`)에 mux 없이
> 직결해 2 Hz 3분 360/360 성공(`notes/data/experiments/20260923_mlx90640-first-real-read.md`).
> §5-1(PT100)은 **첫 판독 성공(2026-09-26)** — 단, 설계와 달리 CS는 **하드웨어 CS0(24번)**, 드라이버는 spidev 직접
> 경로다(D-034). 아래 본문의 '별도 GPIO CS·24번 금지' 서술은 이 결정으로 대체된다. §5-3~§5-7은 **미수행**이다.
> 실행법은 [수집 서비스 README](../jetson/collector/README.md)의 "실기기 점검 도구" 참고.
