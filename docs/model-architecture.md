...
### 2.0 센서 하드웨어 (실물 구성, 2026-07-27 기준)

| 역할 | 실물 | 상태 |
|---|---|---|
| RGB 카메라 (top + oblique 2뷰) | **Sensing ISX031F GMSL2** ×2 (FG12-4CH 어댑터 경유, 1920×1536 UYVY) | **1대 연결 검증 완료** — 버스10(GMSL2 그룹) LinkA = `/dev/video4`. 2대째는 `fzcam_cfg.ini` `position=Video_1100` → `/dev/video5`. 설치법: `cam-adaptor/FG12-4CH-설치방법.md` |
| (임시 대체) USB 웹캠 | 빠른 테스트용 | GMSL2 검증됐으므로 폴백으로만 |
| 열배열 | **MLX90640** 2종 보유 — 표준 55° [220565] / 광각 110° [220573] | I²C. **어느 FOV를 쓸지는 설치 거리 실측 후 선택 (미정)** — 솥 지름과 장착 높이가 정해져야 화각 계산 가능 |
| 중심온도 | **MLX90614** 2종 보유 — 5° [GY-906-DCI] / 35° [GY-906-BCC] | I²C. 5°=원거리 스폿(중심온도 측정 본명), 35°=근거리 넓은 평균. **기본 5° DCI, 실측 후 확정** |

- 모델 입력 관점에서 FOV 변형 선택은 **텐서 shape에 영향 없음**(90640은 어느 쪽이든 32×24,
  90614는 스칼라). 단 ROI 크롭 파라미터와 화소당 물리 면적이 달라지므로 **학습 데이터 수집 전에
  확정**해야 한다(수집 후 교체하면 분포가 틀어짐).
- 열 센서를 FOV 2종 **동시 장착**하는 구성(예: 90640 표준+광각 2채널)은 현재 설계에 없음 —
  채택하려면 thermal branch 입력이 2×24×32가 되므로 이 문서 §3.2를 먼저 개정할 것.
- **FOV 선택 계산법** (지그 설계 확정 시 적용): 거리 d에서 커버 폭 = `2·d·tan(FOV/2)`.
  - 90640 표준 55°×35° → 좁은 축 기준 ≈0.63d (지름 40cm 솥 = 높이 ~63cm 필요)
  - 90640 광각 110°×75° → ≈1.53d (같은 솥 = ~26cm면 충분, 단 증기·김서림 리스크)
  - 90614 5° 스폿 지름 ≈0.09d (60cm에서 ~5cm = 중심온도 용도 부합) / 35° ≈0.63d (전체 평균에 가까움)
  - 원칙: 솥이 프레임을 꽉 채우게 — 남으면 32×24 중 유효 픽셀이 줄어 저해상 센서에 치명적,
    모자라면 끓음 위치·공간 불균일을 놓침. 대략 장착 높이 30cm 이내=광각, 50~70cm=표준 조합.

### 2.1 입력 텐서·주기

| 모달 | 텐서 | 원 주기 | 비고 |
|---|---|---|---|
| RGB oblique(주) | 3×224×224 (1920×1536에서 ROI 크롭·리사이즈) | 1~2 fps | 건더기 무름·국물 탁도 담당 |
| RGB top(보조) | 3×224×224 (동일) | 1~2 fps | 수위·거품·증기·넘침 담당 |
