...
>
> **2026-09-25 갱신(D-030·D-031·D-033):** MLX90614 제외. 온도 스칼라는 솥 내장 센서를 사용하도록
> 계획하며 읽기 경로는 미확인이다. PT100은 개발·검증 및 필요 시 운영 대체용이다.
> 열화상은 D55 한 대 직결. 아래 모델 구조는 설계이며 솥 센서 연동·학습 완료를 뜻하지 않는다.
...
솥 내장 센서 온도 시계열(목표 1Hz, 60s 창)                         │              │
 └─ 파생피처(현재값·dT/dt·온도시간적분) ─▶ MLP ─▶ 64d ──────────────┴──────────────┘
                                                    │
                                                    ▼
...
| (센서 직접) | `center_temp_c`, `thermal.*` | 선택한 온도원의 유효 측정값·열배열. `center_temp_c` 미취득/무효는 `null` |

## 2. 입력 사양과 캐던스

### 2.0 센서 하드웨어 (실물 구성, 2026-09-25 기준)

| 역할 | 실물 | 상태 |
|---|---|---|
| RGB 카메라 (top + oblique 2뷰) | **Sensing ISX031F GMSL2** ×2 (Sensing SG4A 어댑터, 1920×1536 UYVY) | 2대와 Gemini 2의 90.9분 연속 수집 기록 있음. GMSL 오류 플래그는 추가 점검 대상. `notes/data/experiments/20260921_3cam-90min-continuous-run.md` |
| (임시 대체) USB 웹캠 | 빠른 테스트용 | GMSL2 검증됐으므로 폴백으로만 |
| 깊이·컬러·IR 수집 | **Orbbec Gemini 2** | 수집·점검용. 본 문서의 모델 입력 branch에 추가하는 설계는 별도 검토 |
| 열배열 | **MLX90640 D55(55°×35°)** 한 대, D110은 예비 | D-031: J12 3/5번 400 kHz 버스 직결, mux 없음. 지름 40 cm 솥은 높이 64 cm 이상(권장 70 cm) |
| 운영 온도 스칼라 | **솥 내장 온도센서** | D-030: 읽기 인터페이스·측정 위치·갱신 주기 미확인. 시리얼/Modbus 등을 지원한다고 가정하지 않음 |
| 온도 대조·검증 | **MAX31865 + PT100 3선식** | 개발·검증용. 내장 센서를 읽지 못하면 운영 대체원으로 검증. 9/25 현재 MAX31865 통신 미해결(`notes/dev-log.md`) |

- MLX90614는 D-030에 따라 제외했다. 온도 스칼라의 shape은 유지하지만 표면 비접촉 온도와
  내장 센서의 측정 위치·응답 지연은 같지 않다. 기존 온도 임계값·정규화·약지도 기준을 그대로 전용하지 않고
  실제 측정 위치·갱신 주기·PT100 대조를 확인한 뒤 정한다.
- MLX90640 FOV를 교체해도 텐서 shape은 32×24지만 ROI와 화소당 물리 면적이 달라진다.
  설치 높이·ROI·모듈 식별값을 세션에 기록하고, 변경 전후 데이터를 구분한다.
...
- **FOV 선택 계산법** (지그 설계 확정 시 적용): 거리 d에서 커버 폭 = `2·d·tan(FOV/2)`.
  ...
  - 원칙: 솥이 프레임을 꽉 채우게 — 남으면 32×24 중 유효 픽셀이 줄어 저해상 센서에 치명적,
    모자라면 끓음 위치·공간 불균일을 놓침. 현재는 D-031의 D55·높이 64 cm 이상 기준을 적용한다.
...
| MLX90640 열배열 | 16프레임 × 1×24×32 | **2 Hz 수집 목표**, 장치 refresh 8 Hz | 16프레임 창 = 최근 8초. D-032의 무효 프레임은 학습 입력에서 구분 |
| 솥 내장 센서 온도 | 60s 창 (목표 1 Hz × 60) | 실제 갱신 주기 미확인, 1 Hz 입력 목표 | 파생: 현재값, dT/dt, 온도시간적분. PT100 대체 시 온도원 기록 |

- **융합·추론 캐던스 = 1 Hz** (계약의 `cooking/status` 주기와 일치). 이는 모델 입력 목표이며,
  센서가 매초 새 값을 준다는 의미가 아니다. hold-last는 확인된 갱신 주기에 맞는 유효기간 안에서만
  사용하고 원래 취득 시각을 보존한다. 기간을 넘기거나 측정이 무효이면 결손으로 처리한다.
...
### 3.3 스칼라 branch (솥 내장 온도센서 + 시간)
- 60초 창에서 파생피처: 현재 온도, 이동평균 dT/dt, 세션 온도시간적분(∫T dt), 경과시간.
  온도시간적분은 열량(J) 측정값이 아니다.
- 기본 온도원은 솥 내장 센서다. PT100은 대조·검증에 사용하고, 내장 센서를 읽지 못할 경우의
  대체원으로 검증한다. 솥 내장 센서 읽기 경로가 확정되기 전에는 연결된 입력으로 간주하지 않는다.
- 원본과 세션 메타데이터에 온도원·센서 ID·위치·취득 시각·유효성을 남긴다. 열화상 ROI 평균이나
  목표 온도로 스칼라 결손을 메우지 않는다. 출력 필드 `center_temp_c`는 호환용 이름으로 유지한다.
- 2층 MLP → **64d**.
