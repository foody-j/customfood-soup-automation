...
```jsonc
{
  ...
  "center_temp_c": 98.5,                // 솥 내장 센서의 측정 온도(℃); 미취득/무효는 null
...
## 필드 규약
- `center_temp_c`는 호환성을 위해 이름을 유지한 **조리 온도 스칼라**다(D-030·D-033).
  운영 온도원은 솥 내장 센서이며, 읽기 경로·측정 위치·실제 갱신 주기는 아직 미확인이다.
  PT100은 개발·검증용 대조 측정이며 내장 센서를 읽지 못할 때 운영 대체원으로 검증한다.
  선택한 온도원·센서 ID·측정 위치·취득 시각·유효성은 세션 메타데이터와 원본 기록에 남긴다.
  이 필드는 식품 내부 중심온도를 보장하지 않으며, 열화상 중심 화소와도 구분한다.
  열화상 값이나 목표 온도로 자동 대체하지 않는다. 미취득·무효·유효기간 초과 값은 `null`로 보내고
  필요한 온도원이 고장난 경우 `sensor_fault`를 함께 알린다. 화면은 `null`을 '미수신'으로 표시한다.
